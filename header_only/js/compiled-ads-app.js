...
(function () {
  ...
  function syncDesignViewportUnit() {
    // СТАТИЧЕСКИЙ ДИЗАЙН: единица дизайна = живой 1vw. CSS сам пересчитывает её при
    // resize и при браузерном зуме, поэтому раскладка всегда пропорционально
    // заполняет вьюпорт (как в эталоне). Раньше тут «замораживали» значение в px
    // (с множителем зума и через requestAnimationFrame) — это и давало рассинхрон
    // масштаба, смещения и дёрганья при смене масштаба.
    document.documentElement.style.setProperty("--fvw", "1vw");
  }
