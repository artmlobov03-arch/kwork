...
function getBrowserZoomScale() {
  // СТАТИЧЕСКИЙ ДИЗАЙН: не детектим браузерный зум и не компенсируем его.
  // Раскладка целиком на vw + векторный SVG-текст масштабируется сам (как в эталоне),
  // поэтому любой пересчёт под зум только вызывает дёрганья/смещения. Всегда 1.
  return 1;
  const visualScale =
...
function getCurrentPageScale() {
  // Всегда считаем масштаб канвы вживую (текущая ширина / 1920), а не из CSS-переменной,
  // которую обновляет requestAnimationFrame. Так алгоритм переноса строк всегда видит
  // актуальный масштаб и при смене зума не пересчитывает переносы с устаревшим значением.
  return getLayoutViewportWidth() / DESIGN_VIEWPORT_WIDTH;
}
...
function updateSvgTextZoomCompensation() {
  // Всегда векторный SVG-текст: он масштабируется через viewBox+vw без перерисовки,
  // поэтому при зуме нет дёрганий и нет повторной растеризации.
  const nextMode = "svg";
...
function updateZoomAwareLines() {
  ...
  const root = document.documentElement;
  // СТАТИЧЕСКИЙ ДИЗАЙН: толщины линий, точки подчёркивания и обводки радио/чекбоксов
  // выражаем в vw (1 design px = var(--fvw,1vw)/19.2), а не в абсолютных px,
  // пересчитываемых под ширину/зум. Значения идентичны прежним на любой ширине
  // (на 1280px: 12.8/19.2 = 0.667px — как было), но при смене масштаба остаются
  // пропорциональными вьюпорту, поэтому радиокнопки и hairline-границы не «скачут».
  const dpx = function (n) {
    return "calc(" + n + " * var(--fvw, 1vw) / 19.2)";
  };
  // Истинный масштаб канвы (рендер-px / design-px). Нужен алгоритму переноса
  // строк: designW = (rect.width - padding) / pageScale. Величина scale-инвариантна
  // при зуме (rect.width и pageScale меняются пропорционально), поэтому переносы
  // не «прыгают» при смене масштаба, но и не отличаются от оригинала при 100%.
  const pageScale = getLayoutViewportWidth() / DESIGN_VIEWPORT_WIDTH;
  const nextVars = {
    "--page-scale": pageScale.toFixed(6),
    "--dpx": dpx(1),
    "--ui-half-line": dpx(0.5),
    "--ui-hairline": dpx(1),
    "--ui-control-line": dpx(0.5),
    "--ui-control-strong-line": dpx(1.5),
    "--ui-checkbox-line": dpx(1.25),
    "--ui-checkbox-fill-line": dpx(6),
    "--ui-strong-line": dpx(2),
    "--zoom-safe-line": dpx(1),
    "--zoom-dot-size": dpx(2),
    "--zoom-dot-radius": dpx(1),
    "--zoom-dot-edge": dpx(1.1),
    "--zoom-dot-step": dpx(5),
  };
